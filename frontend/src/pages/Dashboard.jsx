...
function Dashboard() {
    ...
    const handleCreateList = async (e) => {
        e.preventDefault();
        if (newListName.trim().length < 2) return showError("El nombre de la lista es demasiado corto");
        ...
        } catch (err) { showError("Error al crear la lista"); }
        finally { setIsLoading(false); } 
    };

    const handleDeleteList = async (id) => {
        if (!window.confirm("¿Seguro que quieres borrar esta lista y sus tareas?")) return;
        setIsLoading(true);
        try {
            await esperar(800);
        ...
        } catch (err) { showError("Error al borrar la lista"); }
        finally { setIsLoading(false); }
    };
    ...
    const handleRenameList = async (id, currentName) => {
        ...
        } catch (err) { showError("Error al renombrar la lista"); }
        finally { setIsLoading(false); }
    };
    ...
    const handleCreateTask = async (e) => {
        ...
        try {
            ...
            if (newTaskReminder) payload.reminderAt = new Date(newTaskReminder).toISOString();
            ...
            setNewTaskReminder('');
        } catch (err) { showError("Error al crear tarea"); }
        finally { setIsLoading(false); }
    };
    ...
    const handleDeleteTask = async (id) => {
        ...
        } catch (err) { showError("Error al borrar tarea"); }
        finally { setIsLoading(false); }
    };
    ...
    const handleUpdateTask = async (id, updates) => {
        ...
        } catch (err) { showError("Error al actualizar tarea"); }
        finally { setIsLoading(false); }
    };
    ...
    useEffect(() => {
        const fetchLists = async () => {
            ...
            try {
                ...
                setLists(res.data);
            } catch (err) { showError("Error al cargar tus listas"); }
            finally { setIsLoading(false); }
        };
        fetchLists();
    }, []);
        ...
        if (selectedList) {
            const fetchTasks = async () => {
                ...
                try {
                    ...
                    setTasks(res.data);
                } catch (err) { showError("Error al cargar las tareas"); }
                finally { setIsLoading(false); }
            };
            fetchTasks();
        } else {
        ...
        <div className="dashboard-page-container">
            <div className="dashboard-wrapper">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', borderBottom: '2px solid #eee', paddingBottom: '10px' }}>
                    ...
                    <button onClick={handleLogout} style={{ background: '#ff4d4d', color: 'white', border: 'none', padding: '10px 15px', borderRadius: '5px', cursor: 'pointer' }}>
                        Cerrar Sesión
                    </button>
                </div>
                ...
                {isLoading && (
                    <div style={{ background: '#e3f2fd', color: '#0d47a1', padding: '10px', borderRadius: '5px', marginBottom: '20px', textAlign: 'center', fontWeight: 'bold' }}>
                ...
                {errorMsg && (
                    <div style={{ background: '#ffebee', color: '#c62828', padding: '10px', borderRadius: '5px', marginBottom: '20px', textAlign: 'center', fontWeight: 'bold', border: '1px solid #ef9a9a' }}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ verticalAlign: 'middle', marginRight: '8px' }}>
                ...
                <div className="dashboard-content" style={{ opacity: isLoading ? 0.6 : 1, pointerEvents: isLoading ? 'none' : 'auto', transition: '0.3s' }}>
                    ...
                    <div className="sidebar">
                        ...
                        <form onSubmit={handleCreateList} className="form-container">
                            <input
                                ...
                                required
                            />
                            <button type="submit" disabled={isLoading} className="btn-modern">Crear</button>
                        </form>
                        ...
                        {lists.map(list => (
                            ...
                            >
                                <span 
                                    ...
                                    style={{ cursor: 'pointer', flex: 1, fontWeight: selectedList === list._id ? 'bold' : 'normal' }}
                                >
                                    {list.name}
                                </span>
                                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                    {/* Icono Editar Lista */}
                                    <button onClick={() => handleRenameList(list._id, list.name)} disabled={isLoading} style={{ cursor: 'pointer', background: 'none', border: 'none', padding: 0, color: '#555', display: 'flex' }}>
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                    </button>
                                    {/* Icono Borrar Lista */}
                                    <button onClick={() => handleDeleteList(list._id)} disabled={isLoading} style={{ cursor: 'pointer', background: 'none', border: 'none', padding: 0, color: '#d32f2f', display: 'flex' }}>
                            ...
                            <>
                                ...
                                <form onSubmit={handleCreateTask} className="form-container">
                                    <input
                                        ...
                                        required
                                    />
                                    <select value={newTaskPriority} onChange={(e) => setNewTaskPriority(e.target.value)} className="input-modern" style={{ flex: 'none', width: '120px' }}>
                                            ...
                                            <li key={task._id} className={`task-item ${task.completed ? 'completed' : ''}`}>
                                                ...
                                                <select 
                                                    ...
                                                    style={{ 
                                                        ...
                                                        color: task.priority === 'high' ? '#d32f2f' : task.priority === 'medium' ? '#ed6c02' : '#2e7d32' 
                                                    }}
                                                >
                                                    <option value="low">Baja</option>
                                                    <option value="medium">Media</option>
                                                ...
                                                {task.reminderAt && (
                                                    <small style={{ color: '#666', fontSize: '12px', display: 'flex', alignItems: 'center' }}>
                                                ...
                                                {/* Icono Borrar Tarea */}
                                                <button onClick={() => handleDeleteTask(task._id)} disabled={isLoading} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#d32f2f', display: 'flex', padding: '5px' }}>
                                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                                                </button>
                                            </li>
                                        ))}
                                    </ul>
                                ) : <p style={{ color: '#666' }}>No hay tareas. ¡Empieza añadiendo una!</p>}
                            </>
                        ) : (
